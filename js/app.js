...
function loadD() {
  try {
    ...
    return {
      ...
      psyProfiles: d.psyProfiles||[], // NOVO
    };
  } catch(e) {
...
function saveD() {
  try { localStorage.setItem('pt', JSON.stringify(D)); } catch(e) {}
}
...
async function requestNotifPermission() {
  if (!('Notification' in window)) {
    toast('⚠️ O teu browser não suporta notificações', 'aw');
    return false;
  }
  ...
  if (granted) {
    ...
    if ('serviceWorker' in navigator) {
      ...
      await reg.pushManager.subscribe({
        ...
        applicationServerKey: urlBase64ToUint8Array('BEl62iUYgUivxIkv69yVeEoi6iBlGVeCwbrW1Olw8cpiyaDzzm0HwcxJ1aUcDXBqS1UN7MNuHFqeQzYgG8yD1')
      });
    }
    ...
    toast(' Permissão negada', 'aw');
  }
}

function disableNotifications() {
  S.notifEnabled = false;
  D.notifEnabled = false;
  saveD();
  toast('🔕 Notificações desativadas', 'ok');
}

function scheduleDailyNotif() {
  if (!S.notifEnabled || !('Notification' in window)) return;
  
  // Verificar se já temos uma notificação agendada
...
function showNotif(title, body, icon = 'icons/icon-192.svg') {
  if (Notification.permission === 'granted') {
...
function go(p) {
  ...
  // Limpar data-game ao voltar ao home
  if (p === 'home') {
...
function checkDailyStreak() {
  ...
  if (last !== today) {
    ...
    if (last === yesterday.toDateString()) {
      ...
      // Quebrou a sequência (ou é primeiro dia)
      if (last !== null) D.streak = 1;
      else D.streak = 1;
    }
...
function applyTh(t) {
  ...
  D.th = t;
  saveD();
  document.getElementById('themeIcon').className = t === 'dark' ? 'bi bi-sun' : 'bi bi-moon-stars';
}
...
function updHome() {
  ...
  if (D.streak >= 7) {
    badge.style.background = 'linear-gradient(135deg, rgba(245,158,11,0.3), rgba(239,68,68,0.3))';
  }
...
function renderGL() {
  const gamesHtml = GAMES.map((g, idx) => {
    const isPsy = g.id === 'psy';
    const classes = isPsy ? 'game-card game-card-featured' : 'game-card';
    
    return `
      <div class="${classes}" style="--card-color:${g.c}" onclick="startG('${g.id}')">
        <div class="game-card-top">
          <div class="game-icon-wrap">${g.i}</div>
          <div class="game-name">${g.n}</div>
        </div>
        <div class="game-card-bottom">
          <span class="game-tag">
            <i class="bi bi-clock-fill"></i> ${g.t}s
          </span>
  ...
  document.getElementById('gList').innerHTML = gamesHtml;
}
...
function renderModalStats() {
  ...
  const best = Math.max(...D.simHistory.map(s => s.score));
  const avg = Math.round(D.simHistory.reduce((a, s) => a + s.score, 0) / D.simHistory.length);
  document.getElementById('mStatBest').textContent = best;
...
function startG(id) {
  ...
  S.start = Date.now(); S.sel = null; S.ans = null;
  
  // Definir atributo data-game no body (para CSS)
  ...
  // Jogos que NÃO usam Confirmar/Saltar
  const noButtons = ['rea', 'mem', 'psy'];
  const gActs = document.getElementById('gActs');
  gActs.style.display = noButtons.includes(id) ? 'none' : 'flex';

  go('game');

  // Perfil Psicológico: lógica totalmente independente
  if (id === 'psy') {
...
function gFig(a) {
  ...
  a.innerHTML = `
    <div class="text-center mb-3"><small class="text-muted">Qual é a próxima figura?</small></div>
    <div class="d-flex justify-content-center flex-wrap gap-2 mb-3">
      ${seq.slice(0, -1).map(f => `<div class="fbox">${rShp(f)}</div>`).join('')}
      <div class="fbox" style="border-style:dashed;color:var(--text-muted);font-size:1.5rem">?</div>
    </div>
    <div class="d-flex justify-content-center flex-wrap gap-2">
      ${opts.map((f, i) => `<div class="fbox ans" onclick="pickOpt(this,${i})">${rShp(f)}</div>`).join('')}
    </div>
  `;
}
...
function gNum(a) {
  ...
  const seq = [];
  let st = Math.floor(Math.random() * 10) + 1;
  ...
  S.ans = opts.indexOf(ans);
  a.innerHTML = `
    <div class="text-center mb-3"><small class="text-muted">Qual é o próximo número?</small></div>
    <div class="d-flex justify-content-center flex-wrap gap-2 mb-3">
      ${seq.slice(0, -1).map(n => `<div class="fbox" style="font-size:1.3rem;font-weight:700">${n}</div>`).join('')}
      <div class="fbox" style="border-style:dashed;font-size:1.3rem;color:var(--text-muted)">?</div>
    </div>
    <div class="d-flex justify-content-center flex-wrap gap-2">
      ${opts.map((n, i) => `<div class="fbox ans" style="font-size:1.3rem;font-weight:700" onclick="pickOpt(this,${i})">${n}</div>`).join('')}
    </div>
  `;
}
...
function gMem(a) {
  ...
  a.innerHTML = `
    <div class="text-center mb-3"><small class="text-muted" id="${isSim?'simMI':'mI'}">Memoriza a sequência...</small></div>
  ...
  setTimeout(() => {
    ...
    document.getElementById('mI').textContent = 'Agora reproduz!';
  }, seq.length * 600 + 800);
...
function pickMem(i) {
  ...
  if (S.memUser.length === S.memSeq.length) {
    let ok = true;
    for (let i = 0; i < S.memSeq.length; i++) {
      if (S.memUser[i] !== S.memSeq[i]) { ok = false; break; }
    }
    S.sel = 0; 
    S.ans = ok ? 0 : 1;
    setTimeout(() => {
...
function gRea(a) {
  ...
  a.innerHTML = `
    <div class="text-center mb-2"><small class="text-muted">Clica quando ficar VERDE!</small></div>
...
function clickReaSim(e) {
  ...
  if (!S.reactReady) {
    ...
    // Atualizar pontuação na simulação
    if (S.sim) {
      const gs = document.querySelector('#simContent .sim-game-header .text-end .fw-bold');
...
function gMat(a) {
  ...
  while (distractors.size < 3 && tries < 50) {
    tries++;
    const magnitude = Math.max(1, Math.abs(ans));
    const range = Math.ceil(magnitude * 0.3) + d * 3;
    const sign = Math.random() < 0.5 ? 1 : -1;
    const offset = sign * (Math.floor(Math.random() * range) + 1);
    const v = ans + offset;
  ...
  a.innerHTML = `
    <div class="text-center mb-3">
      <small class="text-muted">Resolve a operação</small>
      <div style="font-size:2.5rem;font-weight:700;margin-top:.5rem">${x} ${op} ${y} = ?</div>
    </div>
    <div class="d-flex justify-content-center flex-wrap gap-2">
      ${opts.map((n, i) => `<div class="fbox ans" style="font-size:1.5rem;font-weight:700;min-width:80px" onclick="pickOpt(this,${i})">${n}</div>`).join('')}
    </div>
  `;
}
...
function gLog(a) {
  ...
  shuf(opts);
  S.ans = opts.indexOf(ans);
  function rc(f) { return rShpSmall({ s: m[f.s], c: f.c }); }
  function rcBig(f) { return rShp({ s: m[f.s], c: f.c }); }
  a.innerHTML = `
    <div class="text-center mb-2"><small class="text-muted">Completa a matriz 3×3</small></div>
    <div class="d-flex justify-content-center mb-2">
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:.35rem">
        ...
        <div class="lbox" style="border-style:dashed;color:var(--text-muted);font-size:1.2rem">?</div>
      </div>
    </div>
    <div class="d-flex justify-content-center flex-wrap gap-2">
      ${opts.map((f, i) => `<div class="lbox ans" onclick="pickOpt(this,${i})">${rcBig(f)}</div>`).join('')}
    </div>
  `;
}
...
function gAna(a) {
  const analogias = [
    ...
    { a:'COZI NHEIRO', b:'COZINHA', c:'PILOTO', d:'COCKPIT', opts:['COCKPIT','AVIÃO','AEROPORTO','CÉU'] },
    { a:'ATOR', b:'TEATRO', c:'BANCÁRIO', d:'BANCO', opts:['BANCO','DINHEIRO','COFRE','CLIENTE'] },
    
    // Relação animal-característica/som
    { a:'CÃO', b:'LADRAR', c:'GATO', d:'MIAR', opts:['MIAR','RONRONAR','FELINO','PELO'] },
    { a:'LEÃO', b:'RUGIR', c:'LOBO', d:'U IVAR', opts:['UIVAR','ALCATÉIA','NOITE','LUA'] },
    { a:'CABALO', b:'RELINCHAR', c:'BURRO', d:'ZURRAR', opts:['ZURRAR','ORELHAS','CARGA','Teimoso'] },
    ...
    // Relação país-capital
    { a:'PORTUGAL', b:'LISBOA', c:'FRANÇA', d:'PARIS', opts:['PARIS','LYON','MARSALHA','NICE'] },
    { a:'ESPANHA', b:'MADRID', c:'ITÁLIA', d:'ROMA', opts:['ROMA','MILÃO','NAPOLIS','VENEZA'] },
    ...
    { a:'ENADA', b:'NADAR', c:'BICICLETA', d:'PEDALAR', opts:['PEDALAR','RODA','ESTRADA','VELOCIDADE'] },
    
    // Relação estação-característica
    ...
    { a:'SEMENTE', b:'PLANTA', c:'OVO', d:'GALO', opts:['GALO','GALINHA','PINTAINHO','CASC'] },
  ...
  a.innerHTML = `
    <div class="text-center mb-3">
      <small class="text-muted">Completa a analogia</small>
      <div style="font-size:1.3rem;font-weight:700;margin-top:.75rem;line-height:1.7;padding:1rem;background:var(--bg);border-radius:12px;border:1px solid var(--border)">
        <span style="color:var(--primary)">${ana.a}</span> está para <span style="color:var(--primary)">${ana.b}</span>
        <br>como<br>
        <span style="color:var(--primary)">${ana.c}</span> está para <span style="color:var(--danger);font-size:1.5rem">?</span>
      </div>
    </div>
    <div class="d-flex justify-content-center flex-wrap gap-2">
      ${opts.map((o, i) => `
        <div class="fbox ans" style="font-size:.95rem;font-weight:600;min-width:85px;padding:.5rem" onclick="pickOpt(this,${i})">${o}</div>
      `).join('')}
    </div>
  `;
}

/* ============ ROTAÇÃO 3D (DADOS COM PINTAS) ============ */
/* ============ ROTAÇÃO 3D (DADOS - LÓGICA CORRETA) ============ */
function gRot() {
  // Um dado padrão tem faces opostas que somam 7:
  // 1 ↔ 6, 2 ↔ 5, 3 ↔ 4
  // Qualquer rotação mantém estas relações de oposição.
  // Um dado "falso" terá pelo menos um par oposto diferente.
  
  // Estado: [topo, frente, direita, fundo, trás, esquerda]
  // Estado inicial válido: topo=1, frente=2, direita=3
  // (fundo=6, trás=5, esquerda=4 — opostos somam 7)
  const initialState = [1, 2, 3, 6, 5, 4];
  
  // Função para rotacionar o dado num eixo
  function rotate(state, axis, dir) {
    const s = [...state];
    // axis: 'x' (frente↔topo), 'y' (direita↔frente), 'z' (topo↔direita)
    if (axis === 'x') {
      return dir === 'cw'
        ? [s[1], s[3], s[2], s[4], s[0], s[5]]  // frente→topo, fundo→frente, trás→fundo, topo→trás
        : [s[4], s[0], s[2], s[1], s[3], s[5]]; // topo→frente, frente→fundo, fundo→trás, trás→topo
    } else if (axis === 'y') {
      return dir === 'cw'
        ? [s[0], s[2], s[4], s[3], s[5], s[1]]  // direita→frente, trás→direita, esquerda→trás, frente→esquerda
        : [s[0], s[5], s[1], s[3], s[2], s[4]]; // frente→direita, esquerda→frente, direita→trás, trás→esquerda
    } else { // z
      return dir === 'cw'
        ? [s[5], s[1], s[0], s[2], s[4], s[3]]  // esquerda→topo, topo→direita, fundo→esquerda, direita→fundo
        : [s[2], s[1], s[3], s[5], s[4], s[0]]; // direita→topo, fundo→direita, esquerda→fundo, topo→esquerda
    }
  }
  
  // Gerar todas as 24 rotações únicas do dado
  const allRotations = [];
  const seen = new Set();
  ...
  while (queue.length > 0) {
    ...
    // Gerar novas rotações a partir desta
  ...
  // Escolher modelo (estado inicial) e resposta correta (outra rotação)
  ...
  // Gerar dados FALSOS (não são rotações do modelo)
  // Estratégia: trocar duas pintas não-opostas → muda relações de oposição
  function makeFakeDie() {
    // Começar com uma rotação válida
    const base = allRotations[Math.floor(Math.random() * allRotations.length)].slice();
    // Trocar frente e direita (não são opostas)
    // Isto quebra a relação de oposição: se frente=2 e direita=3,
    // após troca: frente=3 e direita=2, mas 3+5=8≠7 e 2+4=6≠7
    [base[1], base[2]] = [base[2], base[1]];
    return base;
  }
  
  // Gerar 3 dados falsos únicos
  const fakes = [];
  ...
  while (fakes.length < 3 && attempts < 50) {
    ...
    const key = fake.join(',');
    // Verificar que não é igual ao modelo nem à resposta
    if (key !== initialState.join(',') && 
        key !== answer.join(',') && 
        !fakeKeys.has(key)) {
      fakeKeys.add(key);
      fakes.push(fake);
  ...
  // Montar opções: 1 correta + 3 falsas, baralhadas
  const opts = [answer, ...fakes];
  shuf(opts);
  S.ans = opts.indexOf(answer);
  
  // Renderizar dado em SVG isométrico
  function renderDice(state, size) {
    ...
    function renderDots(num, dotCx, dotCy, faceSize) {
      ...
      return dots.map(([dx, dy]) => {
        const x = dotCx + dx * faceSize * 0.55;
        const y = dotCy + dy * faceSize * 0.55;
        return `<circle cx="${x}" cy="${y}" r="${dotR}" fill="#1a1a1a"/>`;
      }).join('');
    }
...
function gDom(a) {
  ...
  const sequence = [];
  const start = pieces[Math.floor(Math.random() * pieces.length)];
  sequence.push(start);
  ...
  for (let i = 1; i < seqLen; i++) {
    ...
    if (pattern === 0) {
      // Incrementar top
      next = { top: (sequence[i-1].top + 1) % 7, bottom: sequence[i-1].bottom };
    } else if (pattern === 1) {
      // Incrementar bottom
      next = { top: sequence[i-1].top, bottom: (sequence[i-1].bottom + 1) % 7 };
    } else {
      // Trocar posições
      next = { top: sequence[i-1].bottom, bottom: sequence[i-1].top };
    }
    sequence.push(next);
  }
  ...
  while (opts.length < 4 && safe < 30) {
    ...
    if (!opts.find(o => o.top === p.top && o.bottom === p.bottom)) {
      opts.push(p);
    }
  }
  shuf(opts);
  ...
  function renderPiece(p, size = 50) {
    ...
    const side = (n) => {
      ...
      return `<div style="flex:1;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(3,1fr);place-items:center;padding:2px">
        ${pos.map(p => {
          ...
          return `<div style="${styles[p]}">${dot}</div>`;
        }).join('')}
      </div>`;
    };
    
    return `
      <div style="width:${size}px;height:${size*2}px;background:var(--bg-card);border:2px solid var(--border);border-radius:8px;display:flex;flex-direction:column;overflow:hidden">
        ${side(p.top)}
  ...
  a.innerHTML = `
    <div class="text-center mb-3">
      <small class="text-muted">Completa a sequência de dominó</small>
    </div>
    <div class="d-flex justify-content-center flex-wrap gap-2 mb-3">
      ${sequence.slice(0, -1).map(p => renderPiece(p, 40)).join('')}
      <div style="width:40px;height:80px;border:2px dashed var(--border);border-radius:8px;display:flex;align-items:center;justify-content:center;color:var(--text-muted);font-size:1.5rem">?</div>
    </div>
    <div class="d-flex justify-content-center flex-wrap gap-2">
      ${opts.map((p, i) => `
        <div class="fbox ans" onclick="pickOpt(this,${i})" style="padding:0;overflow:hidden">
          ${renderPiece(p, 35)}
        </div>
      `).join('')}
    </div>
...
function gPsy(a) {
  S.psyQuestions = shuf([...PSY_PERGUNTAS]).slice(0, 10);
  S.psyAnswers = [];
  S.psyCurrentQ = 0;
  psyRender(a);
}
...
function psyRender(a) {
  // Garantir que botões ficam escondidos
  const gActs = document.getElementById('gActs');
  if (gActs) gActs.style.display = 'none';

  // Verificar se terminou
  if (S.psyCurrentQ >= S.psyQuestions.length) {
    psyResult(a);
    return;
  }
  ...
  // Atualizar barra de progresso do header
  const qp = document.getElementById('qP');
  ...
  a.innerHTML = `
    <div class="psy-question">
      <div class="psy-progress">
        <div class="psy-progress-bar" style="width:${pct}%"></div>
      </div>
      <div class="psy-q-num">Pergunta ${num} de ${total}</div>
      <div class="psy-q-text">${q.q}</div>
      <div class="psy-options">
        ${q.opts.map((opt, i) => `
          <button class="psy-opt" onclick="psyPick(${i})">
            <span class="psy-opt-letter">${String.fromCharCode(65 + i)}</span>
            <span class="psy-opt-text">${opt.t}</span>
          </button>
        `).join('')}
      </div>
    </div>
...
function psyPick(idx) {
  ...
  const q = S.psyQuestions[S.psyCurrentQ];
  if (!q) return;
  S.psyAnswers.push(q.opts[idx].p);
  S.psyCurrentQ++;

  // Renderizar próxima pergunta ou resultado
  const a = document.getElementById('gArea');
  if (a) psyRender(a);
}
...
function psyResult(a) {
  ...
  S.psyAnswers.forEach(p => { if (counts[p] !== undefined) counts[p]++; });
  ...
  // NOVO: Guardar perfil identificado
  D.psyProfiles.push({
    profile: dominant,
    date: Date.now()
  });
  ...
  a.innerHTML = `
    <div class="psy-result">
      ...
      <div class="psy-result-section">
        <div class="psy-result-section-title">Pontos Fortes</div>
        <div class="psy-result-tags">
          ${profile.strengths.map(s => `<span class="psy-tag">${s}</span>`).join('')}
        </div>
      </div>

      <div class="psy-result-section">
        <div class="psy-result-section-title">Funções Ideais</div>
        <div class="psy-result-tags">
          ${profile.roles.map(r => `<span class="psy-tag">${r}</span>`).join('')}
        </div>
      </div>

      <div class="psy-result-breakdown">
        <div class="psy-result-section-title">Distribuição de Perfil</div>
        ${Object.entries(counts).map(([key, count]) => {
          const p = PSY_PROFILES[key];
          const pctVal = S.psyQuestions.length > 0 ? Math.round((count / S.psyQuestions.length) * 100) : 0;
          return `
            <div class="psy-breakdown-item">
              <div class="psy-breakdown-label">
                <span>${p.emoji}</span>
                <span>${p.name}</span>
              </div>
              <div class="psy-breakdown-bar">
                <div class="psy-breakdown-fill" style="width:${pctVal}%;background:${p.color}"></div>
              </div>
              <div class="psy-breakdown-pct">${pctVal}%</div>
            </div>
          `;
        }).join('')}
      </div>
      ...
      <div class="d-flex gap-2 mt-4">
        <button class="btn-o flex-grow-1" onclick="go('home')">
          <i class="bi bi-house"></i> Menu
        </button>
        <button class="btn-p flex-grow-1" onclick="startG('psy')">
          <i class="bi bi-arrow-repeat"></i> Refazer
        </button>
      </div>
    </div>
  `;

  toast('🧠 Perfil identificado: ' + profile.name, 'ok');
}


function pickOpt(el, i) {
  // Verificar se estamos na simulação
  const container = el.closest('#simGArea') ? '#simGArea' : '#gArea';
  const parent = document.querySelector(`${container} .ans`).parentElement;
...
function endG() {
  ...
  if (S.sim) {
    ...
    S.simI++;
    if (S.simI < GAMES.length) { setTimeout(() => renderSimTransition(), 400); return; }
    else { renderSimComplete(); return; }
  }
...
function exitGame() {
  ...
  clearTimeout(S.reactTO);
  S.sim = false;
  delete document.body.dataset.game; // Limpar atributo
  go('home');
}
...
function renderSimHeader() {
  const g = GAMES[S.simI];
  const segs = GAMES.map((game, i) => {
    let cls = 'sim-seg';
    let check = '';
    ...
    <div class="sim-header">
      <div class="sim-header-top">
        <div class="sim-header-info">
          ...
          <p class="sim-header-sub">Jogo ${S.simI + 1} de ${GAMES.length}</p>
        </div>
...
function renderSimTransition() {
  ...
  content.innerHTML = `
    ...
    <div class="sim-transition">
      <div class="sim-transition-num">Jogo ${S.simI + 1} de ${GAMES.length}</div>
...
function nextSimQ() {
  ...
  a.classList.remove('fade-in'); void a.offsetWidth; a.classList.add('fade-in');
  ({ fig: gFig, num: gNum, mem: gMem, rea: gRea, mat: gMat, log: gLog })[S.game](a);
}

/* ============ SUBMETER RESPOSTA SIMULAÇÃO ============ */
function submitSimAns() {
  ...
  if (gs) gs.textContent = S.score;
    ...
    // Se não encontrar no header, atualizar no local antigo como fallback
    const oldGs = document.getElementById('gScore');
    if (oldGs) oldGs.textContent = S.score;
  }
...
function startSimNow(id) {
  ...
  // Guardar referência ao jogo atual da simulação
  S.game = id; 
  S.q = 0; 
  S.score = 0; 
  S.correct = 0; 
  ...
  // Renderizar conteúdo específico para simulação
  const content = document.getElementById('simContent');
  content.innerHTML = `
    ${renderSimHeader()}
      ...
      <div class="sim-game-header">
        <div class="sim-game-info">
          ...
          <div>
            ...
            <div class="sim-game-q">Pergunta <span id="simQN">1</span>/<span id="simQT">10</span></div>
          </div>
        </div>
  ...
  // Iniciar temporizador e primeira pergunta
  startSimTm();
  nextSimQ();
}
...
function renderSimComplete() {
  ...
  saveD(); checkAch(); clearInterval(S.simTimer); S.sim = false;
  ...
  const best = D.simHistory.length > 0 ? Math.max(...D.simHistory.map(s => s.score)) : S.simTotalScore;
  ...
  const gamesBreakdown = S.simGameScores.map(gs => {
    ...
    return `
      <div style="display:flex;align-items:center;gap:.75rem;padding:.6rem;background:var(--bg);border-radius:10px;margin-bottom:.4rem;border:1px solid var(--border)">
        ...
        <div style="flex:1">
          <div style="font-weight:600;font-size:.9rem">${game.n}</div>
          <div style="font-size:.75rem;color:var(--text-muted)">${gs.correct}/10 • ${gamePct}%</div>
        </div>
        <div style="font-weight:700;color:var(--primary)">${gs.score}</div>
      </div>
    `;
  }).join('');
  const content = document.getElementById('simContent');
  content.innerHTML = `
    <div class="sim-complete">
      ...
      <div class="sim-complete-rank">
        <div class="sim-complete-rank-title">Classificação</div>
        <div class="sim-complete-rank-v">${rankTitle} • ${pct}%</div>
      </div>
      <div class="sim-complete-stats">
        ...
        <div class="sim-complete-stat"><div class="sim-complete-stat-v">${fmtTime(totalTime)}</div><div class="sim-complete-stat-l">Tempo</div></div>
      </div>
      ...
      <div class="d-flex gap-2">
        ...
        <button class="btn-p flex-grow-1" onclick="openSimModal()"><i class="bi bi-arrow-repeat"></i> Repetir</button>
      </div>
    </div>
  `;
...
function renderPsyStats() {
  ...
  if (D.psyProfiles.length === 0) {
    container.innerHTML = `
      <div style="text-align:center;padding:2rem 1rem;color:var(--text-muted)">
        ...
        <div style="font-size:.85rem">Completa o teste de Perfil Psicológico para ver as tuas estatísticas</div>
      </div>
    `;
    return;
  }
  ...
  // Contar perfis
  const counts = { lider:0, analit:0, social:0, criat:0, exec:0 };
  D.psyProfiles.forEach(p => {
    if (counts[p.profile] !== undefined) counts[p.profile]++;
  });
  
  const total = D.psyProfiles.length;
  const maxCount = Math.max(...Object.values(counts));
  const dominant = Object.keys(counts).find(k => counts[k] === maxCount);
  const dominantProfile = PSY_PROFILES[dominant];
  
  // Último perfil
  const lastProfile = D.psyProfiles[D.psyProfiles.length - 1];
  const lastProfileData = PSY_PROFILES[lastProfile.profile];
  
  container.innerHTML = `
    <div class="psy-stats-summary">
      <div class="psy-stats-dominant">
        ...
        <div class="psy-stats-dominant-info">
          ...
          <div class="psy-stats-dominant-count">${maxCount} de ${total} testes (${Math.round(maxCount/total*100)}%)</div>
        </div>
      </div>
    </div>
    
    <div class="psy-stats-distribution">
      ${Object.entries(counts).map(([key, count]) => {
        const p = PSY_PROFILES[key];
        const pct = total > 0 ? Math.round((count / total) * 100) : 0;
        const isDominant = key === dominant;
        return `
          <div class="psy-stats-row ${isDominant ? 'psy-stats-row-dominant' : ''}">
            <div class="psy-stats-row-label">
              <span class="psy-stats-row-emoji">${p.emoji}</span>
              <span class="psy-stats-row-name">${p.name}</span>
            </div>
            <div class="psy-stats-row-bar-wrap">
              <div class="psy-stats-row-bar" style="width:${pct}%;background:${p.color}"></div>
            </div>
            <div class="psy-stats-row-count">${count} <span class="psy-stats-row-pct">(${pct}%)</span></div>
          </div>
        `;
      }).join('')}
    </div>
    
    <div class="psy-stats-last">
      <div class="psy-stats-last-label">Último resultado</div>
      <div class="psy-stats-last-value">
        <span>${lastProfileData.emoji}</span>
...
function renderStats() {
  ...
  document.getElementById('sSt').textContent = D.bs;
  let tc = 0, tq = 0;
...
function renderCH() {
  ...
  const o = { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: tx } } }, scales: { x: { ticks: { color: tx }, grid: { color: gd } }, y: { ticks: { color: tx }, grid: { color: gd } } } };
  const c1 = document.getElementById('c1');
  if (c1 && h.length > 0) {
    CH.e = new Chart(c1.getContext('2d'), { type: 'line', data: { labels: h.map((_, i) => '#' + (i + 1)), datasets: [{ label: 'Pontos', data: h.map(x => x.s), borderColor: '#4f46e5', backgroundColor: 'rgba(79,70,229,.1)', tension: .3, fill: true }] }, options: o });
  }
  const gs = {}; GAMES.forEach(g => gs[g.id] = { t: 0, c: 0 });
  D.h.forEach(h => { if (gs[h.g]) { gs[h.g].t += h.s; gs[h.g].c++; } });
  const c2 = document.getElementById('c2');
  if (c2) {
    const o2 = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: tx }, grid: { display: false } }, y: { ticks: { color: tx }, grid: { color: gd } } } };
    CH.g = new Chart(c2.getContext('2d'), { type: 'bar', data: { labels: GAMES.map(g => g.i), datasets: [{ label: 'Média', data: GAMES.map(g => gs[g.id].c ? Math.round(gs[g.id].t / gs[g.id].c) : 0), backgroundColor: GAMES.map(g => g.c), borderRadius: 8 }] }, options: o2 });
  }
  const ts = {}; GAMES.forEach(g => ts[g.id] = { t: 0, c: 0 });
  D.h.forEach(h => { if (ts[h.g]) { ts[h.g].t += h.tm; ts[h.g].c++; } });
  const c3 = document.getElementById('c3');
  if (c3) {
    const o3 = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { ticks: { color: tx }, grid: { display: false } }, y: { ticks: { color: tx }, grid: { color: gd } } } };
    CH.t = new Chart(c3.getContext('2d'), { type: 'bar', data: { labels: GAMES.map(g => g.i), datasets: [{ label: 'Tempo (s)', data: GAMES.map(g => ts[g.id].c ? Math.round(ts[g.id].t / ts[g.id].c) : 0), backgroundColor: '#10b981', borderRadius: 8 }] }, options: o3 });
  }
}
...
function renderAch() {
  document.getElementById('aList').innerHTML = ACHS.map(a => {
    ...
    return `<div class="ach ${u ? 'un' : ''}">
      ...
      <div class="flex-grow-1"><div class="fw-bold">${a.n}</div><small class="text-muted">${a.d}</small></div>
      ${u ? '<i class="bi bi-check-circle-fill" style="color:var(--success);font-size:1.3rem"></i>' : '<i class="bi bi-lock" style="color:var(--text-muted)"></i>'}
    </div>`;
  }).join('');
...
function checkAch() {
  const st = { tg: D.tg, ts: D.ts, bs: D.bs, pg: D.pg, hg: D.hg, sm: D.sm, ug: D.ug, br: D.br };
  ACHS.forEach(a => {
...
function renderNotifSettings() {
  ...
  timeInput.value = D.notifTime;
  settings.style.display = D.notifEnabled ? 'block' : 'none';
  
  streakEl.textContent = `${D.streak} dias ${D.streak > 0 ? '🔥' : ''}`;
  streakBar.style.width = `${Math.min(D.streak / 7 * 100, 100)}%`;
}
...
function updateNotifTime() {
  ...
  if (D.notifEnabled) {
    scheduleDailyNotif();
    toast(`⏰ Lembrete agendado para ${time}`, 'ok');
  }
}
